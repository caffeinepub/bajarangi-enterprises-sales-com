...
import { Menu, X, LogIn, LogOut, Settings, Palette } from 'lucide-react';
...
export default function Header() {
  ...
  const navLinks = [
    ...
    { to: '/editor', label: 'Photo Editor', icon: <Palette className="h-3.5 w-3.5" /> },
  ];
        ...
        <nav className="hidden items-center gap-6 md:flex">
          {navLinks.map((link) => (
            <Link
              key={link.to}
              to={link.to}
              className="flex items-center gap-1.5 text-sm font-medium text-foreground/80 transition-colors hover:text-agricultural-green"
